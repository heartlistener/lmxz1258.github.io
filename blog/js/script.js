...
/** This Streams API interface provides聽a built-in byte length queuing strategy that can be used when constructing streams. */
...
/** A CDATA section that can be used within XML to include extended portions of unescaped text. The symbols < and & don鈥檛 need escaping as they normally do when inside a CDATA section. */
...
/** Used by the dataset聽HTML聽attribute to represent data for custom attributes added to elements. */
...
interface EventTarget {
     ...
     *
     * When set to true, options's passive indicates that the callback will not cancel the event by invoking preventDefault(). This is used to enable performance optimizations described in 搂 2.8 Observing event listeners.
     *
...
/** Contains the base URI聽for a document. This object inherits all of the properties and methods as described in the HTMLElement interface. */
...
/** Adds to HTMLElement the properties and methods needed to support basic media-related capabilities聽that are聽common to audio and video. */
...
/** Contains descriptive metadata about a document. It聽inherits all of the properties and methods described in the HTMLElement interface. */
...
interface HTMLOptionsCollection extends HTMLCollectionOf<HTMLOptionElement> {
    ...
    length: number;
    /**
     * Returns the index of the first selected item, if any, or 鈭�1 if there is no selected item.
     *
...
/** This Fetch API interface allows you to perform various actions on HTTP request and response headers. These actions include retrieving, setting, adding to, and removing. A Headers object has an associated header list, which is initially empty and consists聽of zero or more name and value pairs. 聽You can add to this using methods like append() (see Examples.)聽In all methods of this interface, header names are matched by case-insensitive byte sequence. */
interface Headers {
...
/** Allows聽manipulation of聽the browser session history, that is the pages visited in the tab or frame that the current page is loaded in. */
interface History {
    readonly length: number;
...
/** This example shows a variety of different uses of object stores, from updating the data structure with IDBObjectStore.createIndex聽inside an onupgradeneeded function, to adding a new item to our object store with IDBObjectStore.add. For a full working example, see our聽To-do Notifications聽app (view example live.) */
...
/** The聽IIRFilterNode聽interface of the聽Web Audio API聽is a AudioNode processor which implements a general infinite impulse response (IIR)聽 filter; this type of filter can be used to implement tone control devices and graphic equalizers as well. It lets the parameters of the filter response be specified, so that it can be tuned as needed. */
...
/**
 * This EncryptedMediaExtensions API interface represents a聽context for message exchange with a content decryption module (CDM).
 * Available only in secure contexts.
 */
...
/** A stream of media content. A stream consists of several tracks such as聽video or audio tracks. Each track is specified as an instance of MediaStreamTrack. */
...
/**
 * Returns an array of MimeType instances, each of which contains information聽about a supported browser plugins. This object is returned by NavigatorPlugins.mimeTypes.
 * @deprecated
 */
...
/** The OscillatorNode聽interface represents a periodic waveform, such as a sine wave. It is an AudioScheduledSourceNode audio-processing module that causes a specified frequency聽of a given wave to be created鈥攊n effect, a constant tone. */
...
/** PerformanceMark聽is an abstract interface for PerformanceEntry objects with an entryType of "mark". Entries of this type are created by calling performance.mark() to add a named DOMHighResTimeStamp (the mark) to the browser's performance timeline. */
...
/**
 * Used to store a list of Plugin objects describing the available plugins; it's returned by the window.navigator.plugins聽property. The PluginArray is not a JavaScript array, but has the length property and supports accessing individual items using bracket notation (plugins[2]), as well as via item(index) and namedItem("name") methods.
 * @deprecated
 */
...
/** The RTCIceCandidate interface鈥攑art of the WebRTC API鈥攔epresents a candidate Internet Connectivity Establishment (ICE) configuration which may be used to establish an RTCPeerConnection. */
...
/** This WebRTC API interface manages the reception and decoding of data for a聽MediaStreamTrack on an聽RTCPeerConnection. */
...
/** One end of a connection鈥攐r potential connection鈥攁nd how it's configured. Each RTCSessionDescription consists of a description type indicating which part of the offer/answer negotiation process it describes and of the SDP descriptor of the session. */
...
interface Range extends AbstractRange {
    ...
    /** Returns 鈭�1 if the point is before the range, 0 if the point is in the range, and 1 if the point is after the range. */
...
/** The SVGAnimatedString聽interface represents string attributes which can be animated from each SVG declaration. You need to create SVG attribute before doing anything else, everything should be declared聽inside this. */
...
/** SVGTransform is the interface for one of the component transformations within an SVGTransformList; thus, an SVGTransform object corresponds to a single component (e.g., scale(鈥�) or matrix(鈥�)) within a transform attribute. */
...
/** A Selection object聽represents the range of text selected by the user or the current position of the caret. To obtain a Selection object for examination or聽modification, call Window.getSelection(). */
interface Selection {
...
/**
 * The聽ServiceWorkerContainer聽interface of the聽ServiceWorker API聽provides an object representing the service worker as an overall unit in the network ecosystem, including facilities to register, unregister and update service workers, and access the state of service workers and their registrations.
 * Available only in secure contexts.
 */
...
/** A chunk of media to be passed into an HTMLMediaElement and played, via a MediaSource聽object. This can be made up of one or several media segments. */
...
/** A decoder for a specific method, that is a specific character encoding, like utf-8, iso-8859-2, koi8, cp1261, gbk, etc.聽A decoder takes a stream of bytes as input and emits a stream of code points. For a more scalable, non-native library, see StringView 鈥� a C-like representation of strings based on typed arrays. */
...
/** TextEncoder takes a stream of code points as input and emits a stream of bytes. For a more scalable, non-native library, see StringView 鈥� a C-like representation of strings based on typed arrays. */
...
/** Used to represent a set of time ranges, primarily for the purpose of tracking which portions of media have been buffered when loading it for use by the <audio> and <video>聽elements. */
...
/** This Streams API interface provides聽a standard abstraction for writing streaming data to a destination, known as a sink. This object comes with built-in backpressure and queuing. */
...
/** This Streams API interface represents a controller allowing control of a聽WritableStream's state. When constructing a WritableStream, the underlying sink is given a corresponding WritableStreamDefaultController instance to manipulate. */
...
/** The聽XPathEvaluator interface allows to compile and evaluate XPath expressions. */
